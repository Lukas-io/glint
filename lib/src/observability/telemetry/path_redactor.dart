...
/// Masks bearer tokens, JWTs, long hex keys and `password=`-style values.
...
final RegExp _assignmentRegex = RegExp(
  r'''\b((?:password|passwd|pwd|secret|token|api[_-]?key|access[_-]?key|client[_-]?secret)"?)(\s*[:=]\s*"?)(?!<redacted>)[^\s"',;&]+''',
  caseSensitive: false,
);
