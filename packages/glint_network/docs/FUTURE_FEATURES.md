...
## Realtime traffic capture (WebSocket + gRPC + HTTP/2)

Rule: everything is read from the running app through the VM service and DTD. Nothing is added to the app.

- **WebSocket: shipped** (`ws_list`, `ws_get`). dart:io 3.13+ writes `WebSocket.*` timeline events (connect, send, receive, ping, pong, close, error) with direction, type and size. Open: message contents, which the timeline does not carry, and apps on older SDKs.
- **gRPC / HTTP/2: no path yet.** `package:grpc` and `package:http2` run their own HTTP/2 stack over a raw socket, so only `getSocketProfile` byte counters see them. Revisit when the SDK or those packages emit timeline events.
