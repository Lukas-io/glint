...
Per-tool guidance for `glint_network`. Tools are organized in `tools/<category>/` subfolders — each folder has a `README.md` that lists its tools, and each tool has its own page with a **DO NOT USE THIS TOOL WHEN** section at the top. Negative flags catch ~70% of misuse before it happens; read them first.
