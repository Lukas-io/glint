...
flutter_network_mcp sends nothing anywhere unless you turn sharing on.

## What stays on your machine

Every tool call adds a row to the `tool_events` table of the local capture database: the time, the tool name, the outcome (ok, empty or error), the names of the arguments used (never their values), how long it took, the reply size, an estimated token count and, on failure, the error kind. `usage_stats` and `flutter_network_mcp usage` read it back. Set `FLUTTER_NETWORK_MCP_NO_USAGE=true` to stop recording it.

## What is sent if you opt in

Set `FLUTTER_NETWORK_MCP_TELEMETRY=on` to share two kinds of anonymous report with `https://flutter-network-telemetry.wisdomiyamu.workers.dev/v1/telemetry`:
...
Every report is written to the hash-chained `telemetry-audit.log` in the data directory before any network attempt. `flutter_network_mcp usage ship --dry-run` shows the next summary without sending it, `flutter_network_mcp audit show` lists what was recorded, and `flutter_network_mcp audit verify` checks that the log was not edited.

## Switches

| Variable | Effect |
| --- | --- |
| `FLUTTER_NETWORK_MCP_TELEMETRY=on` | Share the anonymous reports. Off unless set. |
| `DO_NOT_TRACK=1` | Never share, even with the opt-in. |
| `FLUTTER_NETWORK_MCP_NO_TELEMETRY=true` | Never share anything. |
| `FLUTTER_NETWORK_MCP_NO_USAGE=true` | Never share usage summaries and stop recording them locally. |
