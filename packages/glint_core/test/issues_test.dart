...
void main() {
  ...
  group('fileWithGh', () {
    CommandRunner script(List<ProcessResult> replies, List<List<String>> calls) {
      var i = 0;
  ...
  test('labels name the package, the kind and agent-filed', () {
    expect(issueLabels(package: 'network', type: 'ux'), ['network', 'ux-friction', 'agent-filed']);
    expect(issueLabels(package: 'glint', type: 'feature'), ['glint', 'enhancement', 'agent-filed']);
    expect(issueLabels(package: 'core', type: 'weird'), ['core', 'agent-filed']);
  });

  test('label filtering keeps the requested order', () {
    expect(selectApplicableLabels(['network', 'bug', 'agent-filed'], {'agent-filed', 'network', 'bug'}),
        ['network', 'bug', 'agent-filed']);
  });

  test('a refused label is recognised in any letter case', () {
    expect(isMissingLabelError("COULD NOT ADD LABEL: 'x' NOT FOUND"), isTrue);
  });
}
